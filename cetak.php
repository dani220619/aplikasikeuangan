...
if( empty( $_SESSION['id_user'] ) ){
   ...
   if(isset($_REQUEST['submit'])){
      ...
      $sql = mysqli_query($con, "SELECT s.nama,p.tgl_bayar,p.jumlah FROM santri s INNER JOIN pembayaran p ON s.nis = p.nis AND p.nis='$nis'");
      list($nama,$tgl_bayar,$jml) = mysqli_fetch_array($sql);
      
      $printTestText = "NIS   : ".$nis."\n";
      $printTestText .= "NAMA  : ".$nama."\n";
      $printTestText .= "Jenis Pembayaran : ".$jenis_pem."\n\n";
      $printTestText .= "========================================\n";
...
<?php
   ...
   $qsiswa = mysqli_query($con, "SELECT * FROM santri WHERE nis='$nis'");
   list($nis,$nama,$idprodi) = mysqli_fetch_array($qsiswa);
   
   echo '<div class="row">';
   echo '<div class="col-sm-6"><table class="table table-bordered">';
   echo '<tr><td colspan="4">Nomor Induk Santri</td><td colspan="3">'.$nis.'</td></tr>';
   echo '<tr><td colspan="4">Nama Santri</td><td colspan="3">'.$nama.'</td></tr>';
   echo '<tr class="info"><th width="50">No</th><th width="100">Kelas</th><th>Bulan</th><th>tahun</th><th>Tanggal Bayar</th><th>Jumlah</th>';
   echo '</tr>';
   
   //tampilkan histori pembayaran, jika ada
   $qbayar = mysqli_query($con, "SELECT jenis_pem,bulan,tahun,tgl_bayar,jumlah FROM pembayaran WHERE nis='$nis' ORDER BY tgl_bayar DESC");
   if(mysqli_num_rows($qbayar) > 0){
      $no = 1;
      while(list($jenis_pem,$bulan,$tahun,$tgl_bayar,$jumlah) = mysqli_fetch_array($qbayar)){
         echo '<tr><td>'.$no.'</td>';
