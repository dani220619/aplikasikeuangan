...
class mypdf extends FPDF{
    ...
    function header(){
        ...
        $this->cell(276,5,'PONDOK PESANTREN AL MUNAWWIR',0,0,'C');
        $this->Ln();
        $this->setfont('Arial','B',12);
        $this->cell(276,10,'KOMPLEK L YOGYAKARTA',0,0,'C');
        $this->Ln(5);
        $this->setfont('Arial','B',12);
        $this->cell(276,15,'Telp. (0411) 878790',0,0,'C');
        $this->Ln(5);
        $this->setfont('Arial','B',12);
    ...
    function viewtable($con, $from, $end){
        ...
        $query=mysqli_query($con,"select * from kas WHERE (tgl BETWEEN '$from' AND '$end') AND jenis='pemasukan' order by tgl asc");
...
@$no++;
            ...
            $this->Cell(50, 7, "".number_format($lihat['debit']).",00", 1, 0,'R');
            $this->Ln();
        ...
        // $this->Ln(5);
            ...
            $this->Cell(272,30,'BENDAHARA',0,1,'R');
        }
